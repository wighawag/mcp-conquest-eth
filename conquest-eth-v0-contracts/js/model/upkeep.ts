...
export function peek(planet: FakePlanet, timeDelta: number) {
	...
	if (planet.externalUpkeep > 0) {
		...
		const decrease = Math.min(
			planet.world.externalUpkeepDownRate * actualTime,
			planet.externalUpkeep,
		);
	...
	if (planet.numSpaceships >= planet.stats.maxUpkeep) {
		planet.numSpaceships =
			planet.numSpaceships - (timeElapsed * planet.stats.production) / 3600 / 2; // TODO
		if (planet.numSpaceships < planet.stats.maxUpkeep) {
		...
		if (timeElapsed > 0) {
			planet.numSpaceships =
				...
				(timeElapsed *
					...
					(Math.max(0, planet.stats.maxUpkeep * timeElapsed - externalArea) /
						(planet.stats.maxUpkeep * timeElapsed))) /
					3600;
		}
...
export function send(planet: FakePlanet, timeDelta: number, quantity: number) {
	...
	if (quantity > planet.numSpaceships) {
		throw new Error(
			`not enough spaceships, only have ${planet.numSpaceships}, cannot send ${quantity}`,
		);
	}
	if (quantity === 0) {
		return;
		...
		if (newDuration > 0) {
			...
			newUpKeep =
				(newDuration * newDuration * planet.world.externalUpkeepDownRate -
					planet.world.externalUpkeepDownRate *
						planet.externalUpkeepDuration *
						planet.externalUpkeepDuration +
					2 * newDuration * quantity +
					2 * planet.externalUpkeepDuration * planet.externalUpkeep) /
				(2 * newDuration);
...
export function countArrival(
	...
	if (planet.externalUpkeep === 0) {
		...
		const newUpKeep =
			(planet.externalUpkeepDuration * planet.externalUpkeep - areaLeft) /
			planet.externalUpkeepDuration;
